...
class DQNAgent:
    ...
    def __init__(self, state_size, action_size, device):
        ...
        # Copy policy weights into target (so they start the same)
    ...
    def pick_action(self, state):
        ...
        With probability 1 - epsilon: pick the best action from the network.
        """
...
def continue_training(checkpoint_path, additional_episodes=2000, render=False):
    ...
    Args:
        checkpoint_path: path to the saved model (e.g., "dqn_results/final_model.pt")
...
if __name__ == "__main__":
    # train(num_episodes=3000, render=False)
    continue_training("dqn_results/final_model.pt", additional_episodes=3000, render=False)

    # To train with visuals (slow, but you can watch):
    # train(num_episodes=100, render=True)

    # To watch a trained agent play:
    # watch("dqn_results/final_model.pt")